...
public class GetGenresRepository: NSObject {
    public typealias GamepediaInstance = (
        GetGenresLocaleDataSource, GetGenresRemoteDataSource
    ) -> GetGenresRepository
    ...
    public init(
        locale: GetGenresLocaleDataSource,
            ...
            .flatMap { result -> AnyPublisher<[GenreDomainModel], Error> in
                if result.isEmpty {
                    return self.remote.getListGenres()
                        .map {
                            GenreTransformer.mapGenresResponsesToEntities(
                                input: $0
                            )
                        }
                        ...
                        .filter { $0 }
                        .flatMap { _ in
                            self.locale.getGenres()
                                .map {
                                    GenreTransformer.mapGenresEntitiesToDomains(
                                        input: $0
                                    )
                                }
                        }
            ...
            .flatMap { result -> AnyPublisher<GenreDomainModel, Error> in
                if result.desc.elementsEqual("Unknown Description") {
                    return self.remote.getGenreDetails(id: id)
                        .map {
                            GenreTransformer.mapGenresResponsesToEntity(
                                input: $0
                            )
                        }
                        .flatMap { res in
                        ...
                        .filter { $0 }
                        .flatMap { _ in
                            self.locale.getDetailGenre(id: id)
                                .map {
                                    GenreTransformer.mapGenresEntityToDomains(
                                        input: $0
                                    )
                                }
                        }
                    ...
                    return self.locale.getDetailGenre(id: id)
                        .map {
                            GenreTransformer.mapGenresEntityToDomains(input: $0)
                        }
                        .eraseToAnyPublisher()
                }
            }.eraseToAnyPublisher()
