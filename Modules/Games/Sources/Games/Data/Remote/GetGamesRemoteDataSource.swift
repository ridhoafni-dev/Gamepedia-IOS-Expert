...
public struct GetGamesRemoteDataSource {
    ...
    func getDiscoveryGames(sortFromBest: Bool) -> AnyPublisher<
        [GameResult], Error
    > {
        let param = [
            "key": apiKey,
            "ordering": sortFromBest == true
                ? orderByRatingDesc : orderByRatingAsc,
        ]
    ...
    func getFewDiscoveryGames() -> AnyPublisher<[GameResult], Error> {
        let param = [
            "key": apiKey, "ordering": orderByRatingDesc, "page_size": "10",
        ]
    ...
    func getGameDetails(id: Int) -> AnyPublisher<DetailGameResponse, Error> {
        ...
        return Future<DetailGameResponse, Error> { @Sendable completion in
            if let url = URL(string: "\(Endpoints.Gets.games.url)/\(id)") {
                AF.request(url, method: .get, parameters: param)
                    .validate()
                    .responseDecodable(of: DetailGameResponse.self) {
                        response in
                        switch response.result {
                        case .success(let data):
