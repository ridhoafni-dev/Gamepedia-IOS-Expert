...
public final class DeveloperTransformer: Mapper {
    ...
    public func transformResponseToEntity(response: [DeveloperResult])
        -> [DeveloperModuleEntity]
    {
        DeveloperTransformer.mapDevelopersResponsesToEntities(input: response)
    }

    public func transformEntityToDomain(entity: [DeveloperModuleEntity])
        -> [DeveloperDomainModel]
    {
        DeveloperTransformer.mapDeveloperEntitiesToDomains(input: entity)
    }

    public func transformResponseToDomain(response: [DeveloperResult])
        -> [DeveloperDomainModel]
    {
        DeveloperTransformer.mapDeveloperResponsesToDomains(input: response)
    }

    static func mapDevelopersResponsesToEntities(
        input developersResponses: [DeveloperResult]
    ) -> [DeveloperModuleEntity] {
        developersResponses.map { result in
    ...
    static func mapDeveloperEntitiesToDomains(
        ...
        developerEntities.map { result in
            DeveloperDomainModel(
                id: Int(result.id),
                name: result.name,
                slug: result.slug,
    ...
    static func mapDeveloperResponsesToDomains(
        ...
        developerResponses.map { result in
            DeveloperDomainModel(
                id: result.id ?? 0,
